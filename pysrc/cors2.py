...
def get_header(h: dict, name: str):
    lname = name.lower()
    for k, v in h.items():
        if k.lower() == lname:
...
def headers_contain_interesting(h: dict) -> bool:
    ...
    acao = get_header(h, "Access-Control-Allow-Origin")
    acac = get_header(h, "Access-Control-Allow-Credentials")

    return (acao in {"*", "null"} and acac is None) or (acao == "*" and acac == "true")
...
def headers_contain_fatal(h: dict) -> bool:
    ...
    return acao == "null" and acac == "true" # okay acao:* + acac:true is no fatal, blocked by spec, modifying


def headers_contain_fatal_2(h: dict) -> bool:
    """Deadly combo: ACAO:* + ACAC:true + X-Cache or similar cache-hit"""
    if not h:
        return False

    acao = get_header(h, "Access-Control-Allow-Origin")
    cache_val = get_header(h, "X-Cache")
    cache_val2 = get_header(h, "Cache-Control")
    
    return acao in {"*", "null"} and (cache_val == "hit" or cache_val2 in {"max-age"} )
...
async def run(urls: List[str]):
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as fh:
        ...
        for r in results:
            ...
            if "error" in r:
                ...
                # continue
                continue
            if headers_contain_interesting(headers) and status_code in [200,201,202,203,204]:
                fh.write(format_record(r))
                matched += 1
    ...
    with open(OUTPUT_FILE2, "w", encoding="utf-8") as fh:
        ...
        for r in results:
            ...
            if "error" in r:
                continue
            if headers_contain_fatal(headers) and status_code in [200,201,202,203,204]:
                fh.write(format_record(r))
                matched2 += 1
    # write fatal records (those with ACAO+Cache headers)
    with open(OUTPUT_FILE3, "w", encoding="utf-8") as fh:
        matched3 = 0
        for r in results:
            headers = r.get("headers", {}) if isinstance(r, dict) else {}
            status_code = r.get("status")
            if "error" in r:
                continue
            if headers_contain_fatal_2(headers) and status_code in [200,201,202,203,204]:
                fh.write(format_record(r))
                matched3 += 1
    ...
    print(f"done — wrote {matched3} matching entries to {OUTPUT_FILE3}")
